...
let newfile = new PNG({ width: 320, height: 224 })
...
function myf3(mytx, myty){

  var tx = mytx; var ty= myty;
fs.createReadStream(__dirname + image_id_url_ts[i][1].substring(1)).pipe(new PNG({filterType: 4})).on("parsed", function() {


for (let y = 0; y < this.height; y++) {
  for (let x = 0; x < this.width; x++) {
    let idx = (this.width * y + x) << 2;
    let idx2 = (newfile.width * (ty + y) + (tx +x)) << 2;
    //let col = (x < newfile.width >> 1) ^ (y < newfile.height >> 1) ? 0xe5 : 0xff;

    newfile.data[idx2] = this.data[idx];
    newfile.data[idx2 + 1] = this.data[idx+1];
    newfile.data[idx2 + 2] = this.data[idx+2];
    newfile.data[idx2 + 3] = this.data[idx+3];
  }
}
howmanydone++;
...
newfile
  .pack()
  .pipe(fs.createWriteStream(__dirname + "/tileset.png"))
  .on("finish", function () {
    console.log("Written!");
  });
}
...
myf2 = function () { 
    if (howmanydone>=image_id_url_ts.length) {
